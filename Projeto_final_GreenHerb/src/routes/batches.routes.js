 ...
 *         description: Não encontrado
 */
router.put('/:id', requireAuth, authorize('Técnico', 'Responsável', 'Administrador'), c.update);
 ...
 */
router.post('/:id/apply-plan', requireAuth, authorize('Técnico', 'Responsável', 'Administrador'), c.applyPlan);
 ...
 */
router.patch('/:id/losses', requireAuth, authorize('Técnico', 'Responsável', 'Administrador'), c.registerLoss);
 ...
 * /batches/{id}/split:
 *   post:
 *     summary: Divide parcialmente um lote ativo
 *     tags: [Batches]
 *     security: [{ bearerAuth: [] }]
 *     parameters:
 *       - in: path
 ...
 *             required: [quantity]
 *             properties:
 *               quantity: { type: number, minimum: 0, description: Quantidade a transferir para o novo lote }
 *     responses:
 *       201:
 *         description: Lote dividido parcialmente; devolve lote original atualizado e novo lote filho
 *       400:
 *         description: Lote não ativo ou quantidade inválida
 *       404:
 *         description: Lote não encontrado
 *       422:
 *         description: Quantidade não pode ser igual/superior à quantidade disponível
 */
router.post('/:id/split', requireAuth, authorize('Técnico', 'Responsável', 'Administrador'), c.splitBatch);

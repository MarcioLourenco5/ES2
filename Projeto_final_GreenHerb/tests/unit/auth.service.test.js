...
describe('PE — Username', () => {
  ...
  test('TU-AU-009 | CE-U1 | username válido + password correcta → 200', async () => {
  ...
  test('TU-AU-010 | CE-U2 | username vazio → 400 USERNAME_INVALID', async () => {
  ...
  test('TU-AU-011 | CE-U3 | username null → 400 USERNAME_REQUIRED', async () => {
  ...
  test('TU-AU-012 | CE-U3 | username undefined → 400 USERNAME_REQUIRED', async () => {
  ...
  test('TU-AU-013 | CE-U4 | username inexistente → 401 CREDENTIALS_INVALID', async () => {
  ...
  test('TU-AU-014 | CE-U5 | username só com espaços → 400 USERNAME_INVALID', async () => {
    const result = await authenticate('   ', 'Admin@1234', mockUsers);
    expect(result.success).toBe(false);
    expect(result.status).toBe(400);
    expect(result.code).toBe(ERRORS.USERNAME_INVALID);
  ...
  test('TU-AU-015 | CE-U6 | utilizador inativo → 403 USER_INACTIVE', async () => {
...
describe('PE — Password', () => {
  ...
  test('TU-AU-016 | CE-P1 | password correcta → 200', async () => {
    bcrypt.compare.mockResolvedValue(true);
    const result = await authenticate('admin', 'Admin@1234', mockUsers);
    expect(result.success).toBe(true);
    expect(result.status).toBe(200);
  ...
  test('TU-AU-017 | CE-P2 | password vazia → 400 PASSWORD_INVALID', async () => {
  ...
  test('TU-AU-018 | CE-P3 | password null → 400 PASSWORD_REQUIRED', async () => {
  ...
  test('TU-AU-019 | CE-P3 | password undefined → 400 PASSWORD_REQUIRED', async () => {
  ...
  test('TU-AU-020 | CE-P4 | password incorrecta (utilizador existe) → 401 CREDENTIALS_INVALID', async () => {
  ...
  test('TU-AU-021 | CE-P5 | password só com espaços → 400 PASSWORD_INVALID', async () => {
    const result = await authenticate('admin', '        ', mockUsers);
    expect(result.success).toBe(false);
    expect(result.status).toBe(400);
    expect(result.code).toBe(ERRORS.PASSWORD_INVALID);
...
test('TU-AU-022 | CE-U2 + CE-P2 | username e password vazios → 400 USERNAME_INVALID', async () => {
  ...
  test('TU-AU-023 | CE-U3 + CE-P3 | username e password null/undefined → 400 USERNAME_REQUIRED', async () => {
